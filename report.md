...
図中のFDBはtrema.multi.confによって定められているものではなく，
multi_learning_switch.rbの仕様を反映させたものである．

単一のスイッチのみで動作する場合とは異なり，複数のスイッチを統合制御する際には
コントローラはスイッチごとにFDBのエントリを管理する必要がある（プログラム中では，これをスイッチのdatapath_idをキーとした連想配列としてFDBを定義することで実現している）．
...
（lsw1とlsw4だけではなく）パケットが通過するすべてのスイッチのフローテーブルに情報が追加される．
